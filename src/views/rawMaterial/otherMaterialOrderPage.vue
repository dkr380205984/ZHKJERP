        ...
        <div class="lineCtn">
          <div class="table">
            ...
            <div class="tableInfo"
              ...
              <span>{{item.stock_number + item.unit}}</span>
            </div>
          </div>
        </div>
      </div>
      <div class="stepCtn">
        <div class="stepTitle">辅料订购</div>
        <div class="borderCtn">
          <div class="cicle"></div>
          <div class="border"></div>
          <div class="cicle"></div>
        ...
        <div class="lineCtn col"
          ...
          <div class="tablePlan">
            ...
            <div class="tableInfo">
              ...
              <span>{{item.selectNum + item.unit}}</span>
            </div>
          </div>
          <div class="buyInfo">
            <ul class="buyFrom"
...
export default {
  ...
  created () {
    this.otherMaterialPlanList.forEach((item, key) => {
      if (key === 0) {
        this.list[0].material = item.material
        this.list[0].needColors.push(item.color)
        this.list[0].needNum = Number(item.plan_number)
        this.list[0].unit = item.unit
      } else {
        let flag = true
        this.list.forEach((value, index) => {
          if (value.material === item.material) {
            flag = false
            value.needColors.push(item.color)
            value.needNum += Number(item.plan_number)
            ...
            obj.needColors.push(item.color)
            obj.needNum = Number(item.plan_number)
            obj.unit = item.unit
            this.list.push(obj)
          }
        })
...
@import "~@/assets/css/otherMaterialOrderPage.less";
</style>
