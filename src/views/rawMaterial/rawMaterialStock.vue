...
<script>
import { orderDetail, rawMaterialOrderList, rawMaterialProcessList, rawMaterialGoStock, rawMaterialGoStockDetail, clientList, notifySave } from '@/assets/js/api.js'
export default {
  data () {
    return {
      localName: 'rawMaterialStock',
      showMsg: false,
      msgFlag: window.localStorage.getItem('rawMaterialStock') ? JSON.parse(window.localStorage.getItem('rawMaterialStock')).msgFlag : false,
      msgUrl: '',
      content: '',
      loading: true,
      order_code: '',
          ...
          rawMaterialGoStock({
            ...
            if (res.status === 200) {
              if (this.msgFlag) {
                ...
                this.content = '订单' + this.order_code + '<span style="color:#1A95FF">物料入库</span>'
                this.sendMsg()
              } else {
                this.$message.success('入库成功')
