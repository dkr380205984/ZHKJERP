...
<script>
import { clientList, productList, productTppeList, flowerList, getGroup, getToken, orderSave, notifySave, orderDetailNew } from '@/assets/js/api.js'
export default {
  data () {
    return {
  ...
  created () {
    Promise.all([clientList({
      ...
      // 初始化数据
      let sampleOrderInfo = res[6].data.data.order_info
      this.orderTitle = sampleOrderInfo.order_code
      this.orderType = sampleOrderInfo.sample_order_type
      this.company = sampleOrderInfo.client_id.toString()
      this.getContacts(this.company)
      this.contacts = sampleOrderInfo.contacts_id
      this.date = sampleOrderInfo.order_time
      this.group = sampleOrderInfo.group_id
      this.presentDate = sampleOrderInfo.order_batch['1'][0].delivery_time
      this.otherInfo = sampleOrderInfo.remark
      let productData = sampleOrderInfo.order_batch[1].map(items => {
        return {
          product_code: items.category_info.product_code,
          category_info: {
            product_category: items.category_info.category_name,
            name: items.category_info.unit
          },
          checked: true,
          color: items.category_info.color,
          id: items.category_info.product_id,
          info: [{ name: [items.size, items.color], numbers: items.numbers }],
          size: items.category_info.size,
          sizeColor: items.category_info.size.map(value => {
            return {
              value: value.measurement,
              label: value.measurement,
              children: items.category_info.color.map(val => {
                return {
                  value: val.color_name,
                  label: val.color_name
                }
          ...
          style_name: items.category_info.style_name,
          type_name: items.category_info.type_name
        }
      })
      productData.forEach(valPro => {
        let flag = this.productArr.find(vals => vals.id === valPro.id)
        if (!flag) {
          this.productArr.push(JSON.parse(JSON.stringify(valPro)))
        } else {
          flag.info.push(...valPro.info)
        }
      })
      this.loading = false
    })
