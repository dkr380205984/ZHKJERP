      ...
      <div class="tableCtn"
        ...
        <div class="tableRow bodyTableRow"
          ...
          <div class="tableColumn"
            ...
            :style="{color:item.has_craft===0?'#b5b5b5':'#1a95ff'}">{{item.has_craft===0?'无':'有'}}</div>
          <div class="tableColumn">{{item.user_name}}</div>
          <div class="tableColumn">{{item.create_time}}</div>
          <div class="tableColumn">
            <span class="btns ban"
              v-if="item.has_plan===1">添加</span>
            <span class="btns normal"
              v-if="item.has_plan===0"
              @click="$router.push('/index/productPlanCreate/'+item.id)">添加</span>
          </div>
        </div>
      </div>
      <div class="pageCtn">
