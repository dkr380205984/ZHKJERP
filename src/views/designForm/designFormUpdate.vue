...
<script>
import { YarnList, editList, YarnColorList, materialList, saveCraft, savePM, PMList, deletePM, craftOne, notifySave } from '@/assets/js/api.js'
...
export default {
  ...
  data () {
    return {
      localName: 'designFormUpdate',
      showMsg: false,
      msgFlag: window.localStorage.getItem('designFormUpdate') ? JSON.parse(window.localStorage.getItem('designFormUpdate')).msgFlag : false,
      msgUrl: '',
      content: '',
      loading: true,
      // 正式数据
      ...
      saveCraft(formData).then((res) => {
        if (res.data.code === 200) {
          if (this.msgFlag) {
            ...
            this.content = '<span style="color:#E6A23C">修改</span>了一张工艺单<span style="color:#1A95FF">' + res.data.data.craft_code + '</span>'
            this.sendMsg()
          } else {
            this.$message.success('修改成功')
